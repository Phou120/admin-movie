...
const { t } = useI18n();
const { updateBanner, upload } = useBanner();
...
watch(
  ...
  (newBanner) => {
    if (newBanner) {
      formData.value = {
        id: newBanner.id,
        file_banner: newBanner.file_banner,
        link: newBanner.link,
        order_by: newBanner.order_by,
      };
      // Set preview image if it exists
      if (newBanner.file_banner && typeof newBanner.file_banner === "string") {
        previewEditImage.value = newBanner.file_banner;
      } else if (newBanner.file_banner instanceof File) {
        // If it's a File object, create a preview
        const reader = new FileReader();
        reader.onload = (e) => {
          previewEditImage.value = e.target?.result as string;
        };
        reader.readAsDataURL(newBanner.file_banner);
      }
    }
  },
  { immediate: true }
...
async function handleSubmit() {
  ...
  loading.value = true;
  try {
    let file_banner: string | File | null = null;
    ...
    const response = await updateBanner({
      ...formData.value,
      file_banner: file_banner as any,
    });
...
<template>
  <a-modal
    :open="visible"
    :title="t('modules.banner.editModal')"
    :footer="null"
    @cancel="handleCancel"
    :width="modalWidth"
    class="banner-modal"
