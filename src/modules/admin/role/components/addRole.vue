<template>
  <div class="add-role-container">
    <div class="page-header">
      ...
      <h1>{{ t("modules.role.addForm.title") }}</h1>
    </div>
        ...
        >
          <a-row :gutter="16">
            <a-col :xs="24" :sm="24" :md="12">
              <a-form-item
                :label="t('modules.role.addForm.name')"
                name="name"
                required
              >
                <a-input
                  v-model:value="formData.name"
                  :placeholder="t('modules.role.addForm.placeholder.name')"
                  size="large"
                />
              </a-form-item>
            </a-col>
            ...
            <a-col :xs="24" :sm="24" :md="12">
              <a-form-item
                :label="t('modules.role.addForm.displayName')"
                name="display_name"
                required
              >
                <a-input
                  ...
                  :placeholder="
                    t('modules.role.addForm.placeholder.displayName')
                  "
                  size="large"
                />
              </a-form-item>
              ...
              <a-form-item name="permission_ids" required>
                <template #label>
                  <span class="permissions-label">
                    ...
                    {{ t("modules.role.addForm.permissions") }}
                  </span>
                </template>
                ...
                >
                  {{
                    t("modules.role.addForm.selectedCount", {
          ...
          <div class="form-actions">
            <a-button size="large" class="custom-cancel" @click="goBack">
              <RollbackOutlined />
              {{ t("modules.role.addForm.actions.goBack") }}
            </a-button>
            <a-button
              type="primary"
              size="large"
            ...
            >
              <SaveOutlined />
              {{ t("modules.role.addForm.actions.createRole") }}
            </a-button>
          </div>
        </a-form>
      </a-card>
...
const rules = computed(() => ({
  name: [
    {
      required: true,
      message: t("modules.role.addForm.validation.nameRequired"),
      trigger: "blur",
    },
    {
      min: 2,
      message: t("modules.role.addForm.validation.nameMinLength"),
      trigger: "blur",
    },
  ],
  display_name: [
    {
      required: true,
      message: t("modules.role.addForm.validation.displayNameRequired"),
      trigger: "blur",
    },
  ],
  permission_ids: [
    {
      ...
      type: "array",
      min: 1,
      message: t("modules.role.addForm.validation.permissionsRequired"),
      trigger: "change",
    },
  ],
}));
...
async function loadAllPermissions() {
  ...
  try {
    ...
    message.error(t("modules.role.addForm.validation.loadPermissionsError"));
  } finally {
...
async function handleSubmit() {
  try {
    ...
    showErrorNotification(
      t("modules.role.addForm.validation.createError"),
      error
    );
  } finally {
